...
${errorInfo.componentStack}`);
  ...
  // extensions/checkout-ui/src/helpers/getCartVariantIds.ts
  function getCartVariantIds(cartLine) {
    const variants = cartLine == null ? void 0 : cartLine.map((item) => {
      var _a, _b, _c;
      return {
        id: (_a = item == null ? void 0 : item.merchandise) == null ? void 0 : _a.id,
        title: (_b = item == null ? void 0 : item.merchandise) == null ? void 0 : _b.title,
        subtitle: (_c = item == null ? void 0 : item.merchandise) == null ? void 0 : _c.subtitle,
  ...
  // extensions/checkout-ui/src/config/sanity.ts
  var config = {
    projectId: "2x6kt394",
    apiVersion: "2022-03-07"
  };

  // extensions/checkout-ui/src/api/fetchSanityVariantsByIds.ts
  function fetchSanityVariantsByIds(ids, isProduction) {
    return __async(this, null, function* () {
      try {
        const response = yield fetch(
          `https://${config.projectId}.api.sanity.io/v${config.apiVersion}/data/query/${// TODO: update boolean by env
          isProduction ? "production" : "production"}?query=*[store.gid in [${ids.join(
            ","
          )}]]{store {gid,},locations[] {"": location -> {countryCode,stockCoverage,_id,},quantity},}`
        );
        const data = yield response.json();
        return data == null ? void 0 : data.result;
      } catch (error) {
  ...
  // extensions/checkout-ui/src/helpers/getNotInStockVariants.ts
  function getNotInStockVariants({
    cartVariantsIds,
    sanityVariants,
    userCountryCode
  }) {
    return cartVariantsIds.filter((cartVariant) => {
      const searchedItem = sanityVariants.find(
        (item) => {
          var _a;
          return ((_a = item == null ? void 0 : item.store) == null ? void 0 : _a.gid) === cartVariant;
        }
      );
      if (searchedItem) {
        const stock = searchedItem == null ? void 0 : searchedItem.locations.find(
          (stockItem) => stockItem.stockCoverage.includes(userCountryCode)
        );
        if (stock && (stock == null ? void 0 : stock.quantity) > 0) {
          return false;
        } else {
          return true;
        }
  ...
  function Extension() {
    ...
    (0, import_react13.useEffect)(() => {
      function fetchSanity() {
        return __async(this, null, function* () {
          return yield fetchSanityVariantsByIds(
            variants == null ? void 0 : variants.map((variant) => `"${variant == null ? void 0 : variant.id}"`),
            // TODO: update boolean by env
            false
          );
        });
      }
      if (Array.isArray(variants) && variants[0].id) {
        setError(null);
        setLoading(true);
        fetchSanity().then((res) => setData(res)).catch((err) => setError(err)).finally(() => setLoading(false));
      }
    }, [countryCode]);
    const unavailableVariants = (0, import_react13.useMemo)(() => {
      var _a;
      if (loading) {
        return [];
      }
      const notInStockVariants = getNotInStockVariants({
        cartVariantsIds: variants == null ? void 0 : variants.map((variant) => variant == null ? void 0 : variant.id),
        sanityVariants: data,
        userCountryCode: countryCode
      });
      return (_a = variants == null ? void 0 : variants.filter((item) => notInStockVariants == null ? void 0 : notInStockVariants.includes(item.id))) == null ? void 0 : _a.map((item) => `${item.title} ${item == null ? void 0 : item.subtitle}`);
    }, [countryCode, loading]);
    useBuyerJourneyIntercept(({ canBlockProgress: canBlockProgress2 }) => {
      if (canBlockProgress2 && (unavailableVariants == null ? void 0 : unavailableVariants.length)) {
        return {
          behavior: "block",
    ...
    if (loading) {
      return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(SkeletonImage2, { inlineSize: "100%", blockSize: 80 });
    }
    if (error) {
      return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Banner2, { title: "Error occur", status: "critical", children: "Sorry something went wrong" });
    }
    return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(import_jsx_runtime4.Fragment, { children: (unavailableVariants == null ? void 0 : unavailableVariants.length) > 0 ? /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(Banner2, { title: "Please note!", status: "critical", children: [
      "Next items in your cart is out of stock:",
      " ",
      unavailableVariants == null ? void 0 : unavailableVariants.join(", ")
    ] }) : /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Banner2, { title: "Success!", status: "success", children: "All items are available in your location" }) });
  }
})();
